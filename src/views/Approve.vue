    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="handleApprove('pass')" type="primary"
            >审核通过</el-button
          >
...
import mitt from "../components/mitt";
export default {
  name: "approve",
  setup() {
    ...
    const getApplyList = async () => {
      ...
      proxy.$nextTick(() => {
        let tbody = document.querySelector(".approve").querySelector("tbody");
        let tds = tbody.querySelectorAll(".cell");
        console.log(tbody);
        let count = tds.length / 11;
        for (let i = 0; i < count; i++) {
          if (tds[10 + i * 11 - 1].innerText === "审批拒绝") {
            tds[10 + i * 11 - 1].style.color = "red";
          } else {
            tds[10 + i * 11 - 1].style.color = "black";
          }
        }
        console.log(tds);
      });
