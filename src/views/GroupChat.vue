        ...
        <div v-for="(message, index) in history.messages" :key="index">
          ...
          <div class="message-recalled" v-if="message.recalled">
            <div v-if="message.senderId !== currentUser.id">
              {{ message.senderData.name }}撤回了一条消息
            </div>
                  ...
                  >
                    ...
                    >
                      <div
                        ...
                        :class="{
                          'play-icon': audioPlayer.playingMessage === message,
                        }"
                      ></div>
                      <div>
...
import GoeasyVideoPlayer from "../components/GoEasyVideoPlayer.vue";
...
export default {
  ...
  created() {
    ...
    this.goEasy.im.on(
      this.GoEasy.IM_EVENT.GROUP_MESSAGE_RECEIVED,
      this.onReceivedGroupMessage
    );
  },
  beforeDestroy() {
    this.goEasy.im.off(
  ...
  methods: {
    ...
    renderMessageDate(message, index) {
      if (index === 0) {
        return utils.formatDate2(message.timestamp);
      } else {
        if (
