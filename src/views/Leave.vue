...
export default {
  ...
  setup() {
    ...
    const getApplyList = async () => {
      ...
      proxy.$nextTick(() => {
        let tbody = document.querySelector(".leave").querySelector("tbody");
        let tds = tbody.querySelectorAll(".cell");
        let count = tds.length / 10;
        for (let i = 0; i < count; i++) {
          if (tds[9 + i * 10 - 1].innerText === "审批拒绝") {
            tds[9 + i * 10 - 1].style.color = "red";
          } else {
            tds[9 + i * 10 - 1].style.color = "black";
          }
        }
        console.log(tds);
      });
